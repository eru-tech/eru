...
type Project struct {
	...
	Auth             map[string]auth.AuthI       `json:"channel"`
...
func (ePrj *ExtendedProject) UnmarshalJSON(b []byte) error {
	...
	if _, ok := ePrjMap["channel"]; ok {
		if ePrjMap["channel"] != nil {
			err = json.Unmarshal(*ePrjMap["channel"], &auths)
			if err != nil {
				logs.WithContext(ctx).Error(err.Error())
				return err
			}
			...
			logs.WithContext(ctx).Info("channel attribute is nil")
		}
	} else {
		logs.WithContext(ctx).Info("channel attribute not found in store")
	}
