...
func (auth *Auth) sendCode(ctx context.Context, credentialIdentifier string, recovery_code string, recovery_time string, name string, projectId string, purpose string, credentialType string) (err error) {
	...
	if purpose == OTP_PURPOSE_RECOVERY {
		...
		if !srcHookFound {
			logs.WithContext(ctx).Warn("SRCF hook not defined for channel. Thus no email was triggered.")
		}
	}
	if purpose == OTP_PURPOSE_VERIFY {
		...
		if !srcHookFound {
			logs.WithContext(ctx).Warn("SVCF hook not defined for channel. Thus no email was triggered.")
		}
	}
	return
}
...
func (auth *Auth) sendWelcomeEmail(ctx context.Context, credentialIdentifier string, name string, projectId string, credentialType string) (err error) {
	...
	if auth.Hooks.SWEF != "" {
		...
		logs.WithContext(ctx).Warn("SWEF hook not defined for channel")
	}
	return
}
...
func (auth *Auth) LoginApi(ctx context.Context, refreshToken string, userId string) (res interface{}, err error) {
	...
	return newToken, nil
	//res, err = channel.Hydra.fetchTokens(ctx, refreshToken)
	//if err != nil {
	//	return
	//}
	//identity, ierr := channel.getUserInfo(ctx, userId)
	//if ierr != nil {
	//		return nil, ierr
	//	}
	//	return channel.makeTokens(ctx, identity)
	return
}
