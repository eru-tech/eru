...
import (
	...
	module_handlers "github.com/eru-tech/eru/eru-channel/module_server/handlers"
...
func SetServiceName() {
	server_handlers.ServerName = "eru-channel"
}
func AddModuleRoutes(serverRouter *mux.Router, sh *module_store.StoreHolder) {

	//store functions specific to files
	//serverRouter.Path("/channel/google/login").HandlerFunc(module_handlers.OauthGoogleLogin())
	//serverRouter.Path("/channel/google/callback").HandlerFunc(module_handlers.OauthGoogleCallback())
	//serverRouter.Path("/channel/openid/login").HandlerFunc(module_handlers.OpenIdLoginHandler(sh.Store))
	//serverRouter.Path("/channel/openid/callback").HandlerFunc(module_handlers.OpenIdCallbackHandler(sh.Store))
	//serverRouter.Path("/channel/openid/getloginflow/{loginchallenge}").HandlerFunc(module_handlers.GetLoginFlowHandlerandler(sh.Store))
	...
	storeRouter.Methods(http.MethodPost).Path("/{project}/save/channel").HandlerFunc(module_handlers.AuthSaveHandler(sh.Store))
	storeRouter.Methods(http.MethodDelete).Path("/{project}/remove/channel/{authname}").HandlerFunc(module_handlers.AuthRemoveHandler(sh.Store))
	...
	authRouter.Methods(http.MethodPost).PathPrefix("/{authname}/login/api").HandlerFunc(module_handlers.LoginApiHandler(sh.Store))
	...
	authRouter.Methods(http.MethodPost).PathPrefix("/{authname}/getusertoken").HandlerFunc(module_handlers.GetUserTokensHandler(sh.Store))
