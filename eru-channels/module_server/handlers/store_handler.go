...
func AuthSaveHandler(s module_store.ModuleStoreI) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			_ = json.NewEncoder(w).Encode(map[string]interface{}{"msg": fmt.Sprint("channel config ", authName, " saved successfully")})
		}
		return
	}
}
...
func AuthRemoveHandler(s module_store.ModuleStoreI) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			s.SaveStore(r.Context(), projectId, "", s)
			server_handlers.FormatResponse(w, 200)
			_ = json.NewEncoder(w).Encode(map[string]interface{}{"msg": fmt.Sprint("channel config ", authName, " removed successfully")})
		}
		return
	}
}
