...
func main() {
	...
	server_handlers.BaseUrl = os.Getenv("ERUQL_PUB_BASE_URL")
	...
	logs.WithContext(ctx).Info("calling SetDataSourceConnections")
	err := sh.Store.SetDataSourceConnections(ctx, sh.Store)
	if err != nil {
		logs.WithContext(ctx).Error(err.Error())
	}
