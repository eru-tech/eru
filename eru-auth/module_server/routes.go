...
func AddModuleRoutes(serverRouter *mux.Router, sh *module_store.StoreHolder) {
	...
	authRouter.Methods(http.MethodPost).PathPrefix("/{authname}/idptoken/{renew}").HandlerFunc(module_handlers.IdpTokenHandler(sh.Store))
	authRouter.Methods(http.MethodPost).PathPrefix("/{authname}/idptoken").HandlerFunc(module_handlers.IdpTokenHandler(sh.Store))
