...
func AddModuleRoutes(serverRouter *mux.Router, sh *module_store.StoreHolder) {
	...
	storeRouter.Methods(http.MethodGet).Path("/{project}/variables/list").HandlerFunc(module_handlers.RouteHandler(sh.Store))
	storeRouter.Methods(http.MethodPost).Path("/{project}/variables/savevar").HandlerFunc(module_handlers.RouteHandler(sh.Store))
	storeRouter.Methods(http.MethodDelete).Path("/{project}/variables/removevar/{key}").HandlerFunc(module_handlers.RouteHandler(sh.Store))
	storeRouter.Methods(http.MethodPost).Path("/{project}/variables/saveenvvar").HandlerFunc(module_handlers.RouteHandler(sh.Store))
	storeRouter.Methods(http.MethodDelete).Path("/{project}/variables/removeenvvar/{key}").HandlerFunc(module_handlers.RouteHandler(sh.Store))
	storeRouter.Methods(http.MethodPost).Path("/{project}/variables/savesecret").HandlerFunc(module_handlers.RouteHandler(sh.Store))
	storeRouter.Methods(http.MethodDelete).Path("/{project}/variables/removesecret/{key}").HandlerFunc(module_handlers.RouteHandler(sh.Store))
