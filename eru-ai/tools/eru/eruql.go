...
type EruqlSaveQueryParams struct {
	...
	Variables    map[string]interface{} `json:"variables" desc:"the variables this query declares, mapped to their DEFAULT values. Name them WITHOUT the $ - the key here is fc, and the query text refers to it as '$fc'. A key written as $fc never matches, because substitution looks for $ plus the key, so the query then runs with the placeholder still in it and fails. A variable the caller omits falls back to its default here; a variable the caller passes - including as an empty string - overrides it, so a default is a fallback for an absent value, not a guard against a blank one. A default may be a go template prefixed with a lowercase none. - for example none.{{ now | date 2006-01-02 }} is today and none.{{ now | dateModify -720h | date 2006-01-02 }} is 30 days ago, with the date layout in quotes (sprig functions are available). Declare every variable the query text uses - an undeclared $name is left in the statement verbatim and the query fails. FIELD_ is graphql-only and a sql query pastes it in literally" default:"{}"`
